...
static lwmqtt_err_t lwmqtt_cycle_until(lwmqtt_client_t *client, lwmqtt_packet_type_t *packet_type, size_t available,
  ...
  //Serial.println("Inside lwmqtt_cycle_until");
  // prepare counter
  size_t read = 0;
  ...
  do {
    // do one cycle
    //Serial.println("Let's call lwmqtt_cycle");
    lwmqtt_err_t err = lwmqtt_cycle(client, &read, packet_type);
    //Serial.println("After lwmqtt_cycle");
    if (err != LWMQTT_SUCCESS) {
      //Serial.println("END lwmqtt_cycle_until returning error");
      return err;
    }
...
lwmqtt_err_t lwmqtt_yield(lwmqtt_client_t *client, size_t available, uint32_t timeout) {
  ...
  //Serial.println("Let's lwmqtt_cycle_until");
  lwmqtt_err_t err = lwmqtt_cycle_until(client, &packet_type, available, LWMQTT_NO_PACKET);
  if (err != LWMQTT_SUCCESS) {
    //Serial.println("END lwmqtt_yield returning Error"); 
    return err;
  }

  //Serial.println("END lwmqtt_yield LWMQTT_SUCCESS");
  return LWMQTT_SUCCESS;
}
