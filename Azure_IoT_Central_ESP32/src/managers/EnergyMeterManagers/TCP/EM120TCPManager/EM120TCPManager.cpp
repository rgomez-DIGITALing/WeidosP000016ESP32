...
EM120TCPManager::EM120TCPManager(ModbusTCPClient& client, IPAddress ip, int deviceId) : EM3PHManager_BaseClass(deviceId) {
  Serial.println("Let's create an EM120-TCP Manager");
  em3ph = new EM3PH_TCP_BasicLine(deviceId, client, ip);
}


EM120TCPManager::~EM120TCPManager(){
  delete em3ph;
}


bool EM120TCPManager::sendProperties(){
  ...
  if(azureDevice->getStatus() == azure_iot_connected){
    ...
    em120Tcp_generete_properties(iotHubClient, payload_buffer, AZ_IOT_DATA_BUFFER_SIZE, &payload_buffer_length, this);
