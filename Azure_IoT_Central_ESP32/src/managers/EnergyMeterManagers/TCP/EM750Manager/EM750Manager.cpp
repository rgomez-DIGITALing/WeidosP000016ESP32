...
EM750Manager::EM750Manager(ModbusTCPClient& client, IPAddress ip, int deviceId) : EM3PHManager_BaseClass(deviceId) {
  Serial.println("Let's create an EM750 Manager");
  em3ph = new EM3PH_TCP_ValueLine(client, ip);
}


EM750Manager::~EM750Manager(){
  delete em3ph;
}
...
bool EM750Manager::sendProperties(){
  ...
  if(azureDevice->getStatus() == azure_iot_connected){
    ...
    em750_generete_properties(iotHubClient, payload_buffer, AZ_IOT_DATA_BUFFER_SIZE, &payload_buffer_length, this);
